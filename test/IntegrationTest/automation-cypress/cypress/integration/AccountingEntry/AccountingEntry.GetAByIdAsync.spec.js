...
describe('API AccountingEntry - GetAByIdAsync', () => {
    ...
    it('GET - AccountingEntry.GetByIdAsync with token null', () => {
        const item = {}
        cy.request({ method: 'GET', url: 'AccountingEntry/GetByIdAsync?id=0AE65F4B-A62E-4929-9371-F1EC6ACC0EF5', body: item, failOnStatusCode: false }).its('status').should('eq', 401)
    })
    ...
    it('GET - AccountingEntry.GetByIdAsync AccountingEntry', () => {
        const item = {

        }
        const header = { "Authorization": "Bearer " + token }
        cy.request({ method: 'GET', url: 'AccountingEntry/GetByIdAsync?id=' + accountingEntryId, body: item, headers: header })
            .should((response) => {
                expect(response.status).to.eq(200)
