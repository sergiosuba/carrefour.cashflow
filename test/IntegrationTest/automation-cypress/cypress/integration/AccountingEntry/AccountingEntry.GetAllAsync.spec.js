...
describe('API AccountingEntry - GetAll', () => {
    ...
    it('POST - GetAllAsync AccountingEntry with token null', () => {
        const item = {}
        cy.request({ method: 'POST', url: 'AccountingEntry/GetAllAsync', body: item, failOnStatusCode: false }).its('status').should('eq', 401)
    })

    it('POST - GetAllAsync AccountingEntry', () => {
        const item = {
            "flow": ""
        }
        const header = { "Authorization": "Bearer " + token }
        cy.request({ method: 'POST', url: 'AccountingEntry/GetAllAsync', body: item, headers: header })
            .should((response) => {
                expect(response.status).to.eq(200)
